...
public class VarcharType extends CharType {
    ...
    public DatabaseDataType toDatabaseDataType(Database database) {
        if ((database instanceof OracleDatabase) || (database instanceof DmDatabase) || ((database instanceof HsqlDatabase) && ((HsqlDatabase) database)
            .isUsingOracleSyntax())) {
            return new DatabaseDataType("VARCHAR2", getParameters());
        }

        if (((database instanceof InformixDatabase) || (database instanceof Gbase8sDatabase)) && (getSize() > 255)) {
