 ...
 * @author mengweijin
 */
public class Gbase8sDatabase extends AbstractJdbcDatabase {

    private static final String PRODUCT_NAME = "GBase 8s Server"; // product name returned by Gbase driver
    ...
    public String escapeObjectName(final String catalogName, final String schemaName, final String objectName, final Class<? extends DatabaseObject> objectType) {
        ...
        if (name.matches(".*\\..*\\..*")) {
            name = name.replaceFirst("\\.", ":"); //Gbase8s uses : to separate catalog and schema. Like "catalog:schema.table"
        }
        return name;
    }
